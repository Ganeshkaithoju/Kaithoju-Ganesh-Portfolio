...
import React, { type FC, useEffect, useMemo, useRef, useState, forwardRef, useImperativeHandle } from "react";
...
function extendMaterial<T extends THREE.Material = THREE.Material>(
  ...
  if ("envMap" in defaults && defaults.envMap !== undefined) uniforms.envMap.value = defaults.envMap;
...
>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    if (mesh.current?.material && "uniforms" in mesh.current.material && mesh.current.material.uniforms?.time) {
...
function SceneGroup({
  ...
  useFrame(() => {
    if (groupRef.current) {
      // Smooth dynamic parallax reaction to page scroll / portfolio length
      const targetRotationZ = degToRad(rotation + scrollProgress * 12);
      const targetPosY = (scrollProgress - 0.5) * 4;
        ...
        THREE.MeshStandardMaterial,
        {
          ...
          vertexHeader: `
            ...
            vec3 getNormal(vec3 pos) {
              ...
              vec3 nextposZ =
                getCurrentPos(
                  pos + vec3(0.0, -0.01, 0.0)
                );

              vec3 tangentX =
                normalize(nextposX - curpos);

              vec3 tangentZ =
                normalize(nextposZ - curpos);
          ...
          vertex: {
            "#include <begin_vertex>": `
              transformed.z +=
                getPos(transformed.xyz);
            `,

            "#include <beginnormal_vertex>": `
              objectNormal =
                getNormal(position.xyz);
            `,
          },
...
export function ThreeDBackground() {
  ...
  useEffect(() => {
    ...
    // Recompute on resize or dynamic content expansion
  ...
  if (!mounted) {
    return (
      <div
        className="fixed inset-0 pointer-events-none -z-50 overflow-hidden bg-black"
        aria-hidden="true"
      />
    );
  }
  ...
  return (
    <div
      className="fixed inset-0 pointer-events-none -z-50 overflow-hidden bg-black select-none"
      aria-hidden="true"
    >
      <Beams
        beamWidth={2.5}
      ...
      {/* Cinematic Vignette & Readability Barrier Overlay */}
