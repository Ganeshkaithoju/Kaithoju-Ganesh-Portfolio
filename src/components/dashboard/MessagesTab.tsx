...
import { Search, Eye, EyeOff, Trash2, Pin, Star, MessageCircle } from "lucide-react";
...
export function MessagesTab() {
  ...
  async function fetchData() {
    try {
      ...
      // Fetch stats
      const { data: allMessages } = await supabase.from('contact_messages').select('status, is_pinned, is_featured');
      const statsObj = {
  ...
  async function updateMessageStatus(
    ...
    try {
      let updateData = {};
      if (action === "approve") updateData = { status: "approved", moderated_at: new Date().toISOString() };
      if (action === "hide") updateData = { status: "hidden", moderated_at: new Date().toISOString() };
      if (action === "pin") {
      ...
      if (action === "delete") {
        ...
        const { error } = await supabase.from('contact_messages').update(updateData).eq('id', messageId);
        if (error) throw error;
      }
      ...
      <div className="mb-8 space-y-4">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 pl-12 text-sm placeholder:text-muted-foreground focus:border-primary/60 focus:outline-none focus:ring-2 focus:ring-primary/30 transition"
          />
        </div>
        ...
        {loading ? (
          ...
          messages.map((msg) => (
            ...
            >
              ...
              <p className="mb-4 leading-relaxed text-muted-foreground whitespace-pre-wrap">{msg.message}</p>
